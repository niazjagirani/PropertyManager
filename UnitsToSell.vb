...
Public Class UnitsToSell
    Public Addnew As Boolean = False
    Dim UpdatePending As Boolean = False
    Dim UpdateDate As String
    Dim SelectAll As Boolean
    Dim message As String
    Dim da As New OleDbDataAdapter("Select * from UnitsTable", Con)
    Dim drb As New OleDbCommandBuilder(da)
    Dim ds As New DataSet

    Private Sub UnitsToSell_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        UnitsDisplay.Show()
    End Sub

    Private Sub UnitsToSell_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
    ...
    Private Sub UnitsToSell_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub ColumnsName()
        Try
            ...
            Arr.Add(New ComboPair("[Unit No]", "Unit No"))
    ...
    Private Sub ImportExcel_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ImportExcel_Button.Click
        Try
            If OFD.ShowDialog() = Windows.Forms.DialogResult.OK Then
                ...
                PathUnits = OFD.FileName.ToString
                If Mid(OFD.FileName, OFD.FileName.Length - 2, 3) = "xls" Then
                    If pro.UnitsFormat = 1 Then
                        OverWriteFrom = 1
    ...
    Private Sub SearchButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SearchButton.Click
        Try
            Dim sql As String = "Select * from UnitsTable where " & SearchCombo.SelectedValue & " like '%" & searchText.Text & "%'"
            Dim Con = New OleDbConnection(My.Settings.MerecConnectionString)
            Dim cmd As New OleDbCommand(sql, Con)
            Dim da As New OleDbDataAdapter(cmd)
            Dim ds As New DataSet
            da.Fill(ds, "UnitsTable")
    ...
    Private Sub PublishSelected_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PublishSelected.Click
        Try
            If Len(UserPass) < 2 Then
                LoginFrom = "UnitsSelected"
                LoginMerec.Show()
            Else

                Dim msg As Integer = MessageBox.Show("Action will delete any published properties from the online server and publish the properties you have selected.", "Publish Message", MessageBoxButtons.YesNo)

                If msg = 6 Then
                    LoginMerec.PublishSelected()
                Else
                End If
            End If
        Catch ex As Exception
            ...
            For n = 0 To DataGrid1.Rows.Count - 2
                ...
                ElseIf CBool(DataGrid1.Rows(n).Cells("chkid").Value) = True Then
                    ...
                    With DataGrid1.Rows(n)
                        ...
                        If .Cells(21).Value.ToString.Length >= 1 Then message = message + " <b>Balcany:</b> " & .Cells(21).Value.ToString() & "<br> "
                        If .Cells(12).Value.ToString.Length >= 1 Then message = message + "<b> Location: </b>" & .Cells(12).Value.ToString() & ", " & .Cells(11).Value.ToString() & "<br> "
                        If .Cells(13).Value.ToString.Length >= 1 Then message = message + " <b>Unit No: </b>" & .Cells(13).Value.ToString() & "<br>  "
                        If .Cells(14).Value.ToString.Length >= 1 Then message = message + " <b>Unit Type: </b>" & .Cells(14).Value.ToString() & "<br>"
                        If .Cells(16).Value.ToString.Length >= 1 Then message = message + " <b>Floor: </b>" & .Cells(16).Value.ToString() & "<br> "
                        If .Cells(17).Value.ToString.Length >= 1 Then message = message + " <b>View: </b> " & .Cells(17).Value.ToString() & "<br> "
                        If .Cells(18).Value.ToString.Length >= 1 Then message = message + " <b>Area / Size: </b>" & .Cells(18).Value.ToString() & " " & .Cells(20).Value.ToString() & "<br> "
                        If .Cells(19).Value.ToString.Length >= 1 Then message = message + " <b>Land Area: </b>" & .Cells(19).Value.ToString() & " " & .Cells(20).Value.ToString() & "<br> "
                        If .Cells(15).Value.ToString.Length >= 1 Then message = message + " <b>Bed Rooms:</b> " & .Cells(15).Value.ToString() & "<br>"
                        If .Cells(26).Value.ToString.Length >= 1 Then message = message + " <b>Completion: </b>" & .Cells(26).Value.ToString() & "<br> "
                        If .Cells(25).Value.ToString.Length >= 1 Then message = message + " <b>Status:</b> " & .Cells(25).Value.ToString() & "<br> "
                        If .Cells(22).Value.ToString.Length >= 1 Then message = message + " <b>Original Price:</b> " & .Cells(22).Value.ToString() & " " & .Cells(24).Value.ToString() & "<br> "
                        If .Cells(23).Value.ToString.Length >= 1 Then message = message + " <b>Selling Price:</b> " & .Cells(23).Value.ToString() & " " & .Cells(24).Value.ToString() & "<br>"
                        If .Cells(34).Value.ToString.Length >= 1 Then message = message + " <b> Notes: </b>" & .Cells(34).Value.ToString() & "<br>"

                        path2 = .Cells(36).Value.ToString()
                    End With
    ...
    Private Sub QSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FullList.Click
        Try
            ...
            Dim da22 As New OleDbDataAdapter("select * from unitstable where [Community/Project] like '%" _
    ...
    Private Sub Undo10mnts_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
            ...
            End If
            Dim cmd2 As New OleDbCommand("Select * from UnitsTable", Con2)
            Dim da2 As New OleDbDataAdapter(cmd2)
            Dim Con = New OleDbConnection(My.Settings.MerecConnectionString)
            Con.open()
            Dim cmd As New OleDbCommand("Select * from UnitsTable", Con)
            Dim da As New OleDbDataAdapter(cmd)
            Dim ds, ds2 As New DataSet
            da.Fill(ds, "UnitsTable")
            da2.Fill(ds2, "UnitsTable")

            ds.Tables.Remove(ds.Tables("UnitsTable"))
            ds.Tables.Add(ds2.Tables("UnitsTable").Copy)
            Dim cb As New OleDbCommandBuilder(da)

            DataGrid1.DataSource = ds.Tables("UnitsTable")
            DataGrid1.Refresh()
            Dim dt As DataTable = ds.Tables("UnitsTable")
    ...
    Private Sub FullListButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FullListButton.Click
        Try
            DataGrid1.DataSource = UnitsTableTableAdapter.GetData
            DataGrid1.Refresh()
        Catch ex As Exception
        End Try
    End Sub
