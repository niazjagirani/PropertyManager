...
Public Class LandDisplay
    Dim img1, img2, img3, img4 As String
    Dim imgtf1, imgtf2, imgtf3, imgtf4 As Boolean


    Private Sub UnitsDisplay_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            AttachFrom = "Land"
            Call FillTree()
            Dim pf As New ExtraFuntions
            pf.ProjectFuction(ProjectName, "Land")
            UnitEnteredBy.Text = UserPass
        Catch ex As Exception
            ' MessageBox.Show(ex.ToString)
            ' MessageBox.Show("Error found in Land startup section. please contact with comments@estabase.com", "Start up error", MessageBoxButtons.OK)
        End Try
    ...
    Private Sub FillTree()
        Try
            ...
            Dim daProjects As New OleDbDataAdapter("Select * from Projects where projecttype='Land' order by [project name] asc", Con)
            Dim daUnits As New OleDbDataAdapter("Select * from LandTable ", Con)
            Dim daUnitsEmpty As New OleDbDataAdapter("Select * from LandTable where [Project ID]<1", Con)
            Dim dS As New DataSet
            Dim dr, dr2 As DataRow
            daProjects.Fill(dS, "Projects")
            daUnits.Fill(dS, "Landtable")
            ...
            For Each dr In dS.Tables("Projects").Rows
                ...
                For Each dr2 In dS.Tables("LandTable").Rows
                    If dr("Project ID") = dr2("Project ID") Then
                        If dr2("Parcel No").ToString.Length < 1 Then
                            str = "NA"
                        Else

                            str = dr2("Parcel No").ToString
                        End If
    ...
    Private Sub DatainView_Unit()
        Try
            ...
            Dim dr As DataRow
            daUnits.Fill(dS, "LandTable")
            For Each dr In dS.Tables("LandTable").Rows
                If dr("Property ID") = TreeView1.SelectedNode.Tag Then
                    PercelNo.Text = dr("Parcel No").ToString
                    LandSize.Text = dr("Land Size").ToString
                    LandHieght.Text = dr("Allowed Height").ToString
                    LandArea.Text = dr("Allowed builtup Area").ToString
                    LandSqrorMtr.Text = dr("SqrFt/SqrMt").ToString
                    PropertyType.Text = dr("Property type").ToString
                    LandFar.Text = dr("far").ToString
                    UnitSellingPrice.Text = dr("Selling Price").ToString
                    UnitOriginalPrice.Text = dr("Original Price").ToString
    ...
    Private Sub Update_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Update_Button.Click
        Try
            ...
            If PropertyType.Text = Nothing Or UnitSellingPrice.Text = Nothing Or UnitNameOfSource.Text = Nothing Then
            ...
            Else
                ...
                Dim SQL As String
                SQL = "update LandTable set " _
                & "[Project ID]       =" & ProjID _
                 & ", [Master Project]         ='" & UnitCommunity.Text _
                 & "', [Community/project]   ='" & ProjectName.Text _
                 & "', [Property Type]       ='" & PropertyType.Text _
                 & "', [Phase]               ='" & UnitPhase.Text _
                 & "', [Address]             ='" & UnitsAddress.Text _
                 & "', [Country]             ='" & UnitCountry.Text _
                 & "', [City]                ='" & UnitCity.Text _
                 & "', [Parcel No]             ='" & PercelNo.Text _
                 & "', [Land Size]           ='" & LandSize.Text _
                 & "', [Allowed Height]      ='" & LandHieght.Text _
                 & "', [Allowed builtup area]               ='" & LandArea.Text _
                 & "', [SqrFt/SqrMt]         ='" & LandSqrorMtr.Text _
                 & "', [FAR]             ='" & LandFar.Text _
                 & "', [Original Price]      ='" & UnitOriginalPrice.Text _
                 & "', [Selling Price]       ='" & UnitSellingPrice.Text _
                 & "', [Currency]            ='" & Currency.Text _
    ...
    Private Sub Save_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Save_Button.Click
        Try
            ...
            Else
                ...
                SQL = "select count([Property ID])  from [LandTable]"
                objCmd = New OleDbCommand(SQL, Con)
                vl = objCmd.ExecuteScalar
                If vl > 0 Then
                    SQL = "select max([Property ID])  from [LandTable]"
                    objCmd = New OleDbCommand(SQL, Con)
                ...
                SQL = "insert into landtable ([Project ID],[Property ID], [User ID],[Master Project], [Community/project], [Property Type] " _
                                      ...
                                      & PropertyType.Text & "','" & UnitPhase.Text & "','" & UnitsAddress.Text & "','" & UnitCountry.Text & "','" & UnitCity.Text & "','" & _
                                       PercelNo.Text & "','" & LandArea.Text & "','" & LandHieght.Text & "','" & LandArea.Text & "','" & _
                                       LandSqrorMtr.Text & "','" & LandFar.Text & "','" & UnitOriginalPrice.Text & "','" & UnitSellingPrice.Text & "','" & Currency.Text & "','" & _
                                        UnitEnteredBy.Text & "','" & AgentSeller.Text & "','" & UnitNameOfSource.Text & "','" & _
    ...
    Private Sub DeleteProperty_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DeleteProperty.Click
        Try
            Dim cmd As New OleDbCommand("delete * from LandTable where [Property ID]=" & TreeView1.SelectedNode.Tag, Con)
            cmd.ExecuteNonQuery()
    ...
    Public Sub FileRet()
        Try
            ...
            Dim cmdList As New OleDbCommand("Select * from filesTable where [property id]=" & TreeView1.SelectedNode.Tag & " and [File Source] = 'Land' ", Con)
    ...
    Private Sub DeleteAttached_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DeleteAttached.Click
        Try
            If FileList.SelectedItem.ToString.Length >= 1 Then
                ...
                Dim cmdList As New OleDbCommand("delete from filesTable where [property id]=" & TreeView1.SelectedNode.Tag & " and [File Source] = 'Land' and [File Name]='" & FileList.SelectedItem.ToString & "'", Con)
    ...
    Private Sub ParcelMessage_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ParcelMessage.Click
        MessageShow.TextBox1.Text = " Enter the lot or parcel number acquired on your land by the authority (Land Department, Master Developer), Example: (121-133), or (BB.A01-001)."
    ...
    Private Sub SizeMessage_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SizeMessage.Click
        MessageShow.TextBox1.Text = " Enter the area of the land in square feet or square meters"

        MousePos()
        MessageShow.Show()
    End Sub

    Private Sub HeightMessage_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles HeightMessage.Click
        MessageShow.TextBox1.Text = " Enter the maximum permitted height allowed on this land by the authority, Example: G+1, or 2 floors, or 120meters."
    ...
    Private Sub BuiltMessage_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BuiltMessage.Click
        MessageShow.TextBox1.Text = " Enter the maximum allowed area of construction allowed on this land provided by the authorities and shown on the affection plan, if the FAR (Floor Area Ratio) is only displayed on the Affection Plan, multiply it by the Land area to enter the result here."
    ...
    Private Sub FARMessage_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FARMessage.Click
        MessageShow.TextBox1.Text = "The ratio described by the Master Developer or the Authority (Land Department) by which you multiply the land area to give you the gross built-up area ie: FAR=GFA/Land Area"
